...
// ---- LINE Webhook（最初に定義：body-parserをかけない）----
app.post("/line-webhook", line.middleware(lineConfig), async (req, res) => {
  ...
  for (const ev of req.body.events) {
    try {
      if (ev.type === "message" && ev.message.type === "text") {
        ...
        const gpt = await ai.chat.completions.create({
          model: "gpt-4o-mini",
          messages: [
            { role: "system", content: "You are K, reply concisely in Japanese for restaurant/spa ops." },
    ...
    } catch (e) { console.error("❌ LINE処理エラー:", e?.message || e); }
  }
});

// ---- ここから先でJSON/URLENCODEDを使う（WhatsApp用）----
